...
class _ResultScreenState extends State<ResultScreen>
  ...
  Future<void> _loadRateInfo() async {
    context.read<ConverterBloc>().add(GetCurrentRateRequested(
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ConverterBloc, ConverterState>(
      builder: (context, state) {
        // Update rate info when we get a successful rate response
        if (state is ConverterRateSuccess) {
          _currentRate = state.rate;
          _isRateCached = state.isCached;
          _rateTimestamp = state.timestamp;
        }
                          ...
                          children: [
                            ...
                            if (_isRateCached)
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: _isRateCached && widget.isStale 
                                    ? Colors.red.withOpacity(0.2)
                                    : Colors.orange.withOpacity(0.2),
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                child: Text(
                                  _isRateCached && widget.isStale ? 'Stale Cache' : 'Cached',
                                  style: TextStyle(
                                    fontSize: 12,
                                    color: _isRateCached && widget.isStale 
                                      ? Colors.red 
